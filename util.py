...
def construct_test_statistic_w(j, XA, y, A, W):
    ej = []
    for each_j in A:
        if j == each_j:
            ej.append(1)
...
def compute_yz_w(y, etaj, zk, n, W):
    W_inv = np.linalg.pinv(W)
    p2 = W_inv @ etaj @ np.linalg.pinv(etaj.T @ W_inv @ etaj) @ etaj.T
    e1 = np.identity(n) - p2
    a = np.dot(e1, y)
    b = W_inv @ etaj @ np.linalg.pinv(etaj.T @ W_inv @ etaj) 
    yz = a + b*zk
